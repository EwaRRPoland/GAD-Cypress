...
// data - articles
import { article1Data, article1DataPATCH } from '../../support/data/apiDataArticles';
import { article2Data, article2DataPATCH } from '../../support/data/apiDataArticles';
import { article3Data, article3DataPATCH } from '../../support/data/apiDataArticles';
...
it('response code should be 201 and should create article3', () => {
    ...
    // update article1 PATCH article1DataPATCH
it('response code should be 200 and should update article1DataPATCH', () => {
    cy.request({
        method: 'PATCH',
        url: `http://localhost:3000/api/articles/${articleId1}`,
        body: article1DataPATCH,
        headers: {
            Authorization: `Bearer ${accessToken1}`
        },
        failOnStatusCode: false
        ...
        assert.isNotNull(articleId1, 'Article ID should not be null');
        assert.containsAllKeys(response.body, Object.keys(article1DataPATCH), 'Response body should contain the updated fields');
        cy.log('Updated article ID:', articleId1);
    });
});
    // update article2 PATCH article2DataPATCH
it('response code should be 200 and should update article2DataPATCH', () => {
    cy.request({
        method: 'PATCH',
        url: `http://localhost:3000/api/articles/${articleId2}`,
        body: article2DataPATCH,
        headers: {
            Authorization: `Bearer ${accessToken2}`
        },
        failOnStatusCode: false
        ...
        assert.equal(status, 200, 'Response status should be 200');
        assert.isNotNull(articleId2, 'Article ID should not be null');
        assert.containsAllKeys(response.body, Object.keys(article2DataPATCH), 'Response body should contain the updated fields');
        cy.log('Updated article ID:', articleId2);
    });
});
    // update article3 PATCH article3DataPATCH
it('response code should be 200 and should update article3DataPATCH', () => {
    cy.request({
        method: 'PATCH',
        url: `http://localhost:3000/api/articles/${articleId3}`,
        body: article3DataPATCH,
        headers: {
            Authorization: `Bearer ${accessToken3}`
        },
        failOnStatusCode: false
        ...
        assert.equal(status, 200, 'Response status should be 200');
        assert.isNotNull(articleId3, 'Article ID should not be null');
        assert.containsAllKeys(response.body, Object.keys(article3DataPATCH), 'Response body should contain the updated fields');
